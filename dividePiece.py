...
total_derivative = []  # 导数
total_guest_score = []  # 客队得分
total_host_score = []  # 主队得分
...
def mysql_query(results):
    ...
    for row in results:
        ...
        if row.section_num == section_number:
            ...
            guest_s = int(row.score.split('-')[0])
            host_s = int(row.score.split('-')[1])
            # # 打印结果
...
Slice = namedtuple('Slice', ['begin_time', 'end_time', 'state', 'begin_host_score', 'end_host_score', 'begin_guest_score', 'end_guest_score'])
    ...
    for i in range(0, len(total_derivative)):  # 在第i节中
        ...
        for k in range(0, len(total_derivative[i])):
            if judge_symbol(total_derivative[i][k]) == -state:
                ...
                # 赋值
                s = Slice(begin_time, end_time, state, begin_host_s, end_host_s, begin_guest_s, end_guest_s)
                slice_set.append(s)
        ...
        if slice_set[-1].end_time != 0:
            s = Slice(slice_set[-1][1], 0, state, slice_set[-1].end_host_score, total_host_score[i][-1], slice_set[-1].end_guest_score, total_guest_score[i][-1])
            slice_set.append(s)
...
Compose = namedtuple('Slice', ['begin_time', 'end_time', 'state', 'begin_host_score', 'end_host_score', 'begin_guest_score', 'end_guest_score'])
...
def data_compose():
    for i in range(0, len(total_slice_set)):  # 在每一节中
        ...
        max_difference = total_slice_set[i][0].end_guest_score - total_slice_set[i][0].end_host_score
        ...
        for k in range(0, len(total_slice_set[i])):
            if total_slice_set[i][k].state != state:
                ...
                # 该if为下一区间加入分片的情况
                if (state == 1 and total_slice_set[i][k].end_guest_score - total_slice_set[i][k].end_host_score >= max_difference) or (state == -1 and total_slice_set[i][k].end_guest_score - total_slice_set[i][k].end_host_score <= max_difference):  # 增函数区间
                    max_difference = total_slice_set[i][k].end_guest_score - total_slice_set[i][k].end_host_score
                ...
                # 该为到达该总分片终点
                    ...
                    c = Compose(begin_time, end_time, state, begin_host_s, end_host_s, begin_guest_s, end_guest_s)
                    # 加入列表中
        ...
        if len(final_slice_set) == 0:
            c = Compose(720, 0, state, total_host_score[i][0],
                        total_host_score[i][-1], total_guest_score[i][0], total_guest_score[i][-1])
            final_slice_set.append(c)
        # 若最后分片没有把末尾加入，则手动加入至末尾
        if final_slice_set[-1].end_time != 0:
            c = Compose(final_slice_set[-1].end_time, 0, state, final_slice_set[-1].end_host_score, total_host_score[i][-1], final_slice_set[-1].end_guest_score, total_guest_score[i][-1])
            # 加入列表中
            final_slice_set.append(c)
                ...
                for m in range(0, len(total_time[i])):
                    if (total_time[i][m] < final_total_slice_set[i][n].begin_time) and (total_time[i][m] > final_total_slice_set[i][n].end_time) and (total_guest_score[i][m] - total_host_score[i][m] <= 0) and (total_guest_score[i][m+1] - total_host_score[i][m+1] > 0):
                        ...
                        c = Compose(final_total_slice_set[i][n].begin_time, total_time[i][m],
                                    ...
                                    total_host_score[i][m], final_total_slice_set[i][n].begin_guest_score,
                                    total_guest_score[i][m])
                        final_total_slice_set[i].insert(n + 1, c)
                        del final_total_slice_set[i][n]
                        break
            elif state == -1 and final_total_slice_set[i][n].begin_guest_score - final_total_slice_set[i][n].begin_host_score > threshold and final_total_slice_set[i][n].end_guest_score - final_total_slice_set[i][n].end_host_score < -threshold:
                for m in range(0, len(total_time[i])):
                    if (total_time[i][m] < final_total_slice_set[i][n].begin_time) and (total_time[i][m] > final_total_slice_set[i][n].end_time) and (total_guest_score[i][m] - total_host_score[i][m] >= 0) and (total_guest_score[i][m+1] - total_host_score[i][m+1] < 0):
                        c = Compose(total_time[i][m], final_total_slice_set[i][n].end_time, final_total_slice_set[i][n].state, total_host_score[i][m], final_total_slice_set[i][n].end_host_score, total_guest_score[i][m], final_total_slice_set[i][n].end_guest_score)
                        final_total_slice_set[i].insert(n + 1, c)
                        c = Compose(final_total_slice_set[i][n].begin_time, total_time[i][m], final_total_slice_set[i][n].state, final_total_slice_set[i][n].begin_host_score, total_host_score[i][m], final_total_slice_set[i][n].begin_guest_score, total_guest_score[i][m])
                        final_total_slice_set[i].insert(n + 1, c)
                        del final_total_slice_set[i][n]
                        break
...
def get_type():
    for i in range(0, len(final_total_slice_set)):
        ...
        for k in range(0, len(final_total_slice_set[i])):
            ...
            # 焦灼
            if abs((final_total_slice_set[i][k].begin_guest_score - final_total_slice_set[i][k].begin_host_score) - (final_total_slice_set[i][k].end_guest_score - final_total_slice_set[i][k].end_host_score)) < anxious_value:
                t_type = 3
            elif state == 1:
                # 客队领先并扩大优势
                if final_total_slice_set[i][k].begin_guest_score - final_total_slice_set[i][k].begin_host_score >= threshold:
                    t_type = 0
                # 客队建立优势
                elif threshold >= final_total_slice_set[i][k].begin_guest_score - final_total_slice_set[i][k].begin_host_score > -threshold:
                    t_type = 1
                # 客队落后但缩小分差
                elif final_total_slice_set[i][k].begin_guest_score - final_total_slice_set[i][k].begin_host_score < -threshold:# and final_total_slice_set[i][k].end_guest_score - final_total_slice_set[i][k].end_host_score < threshold:
                    t_type = 2
            elif state == -1:
                # 主队领先并扩大优势
                if final_total_slice_set[i][k].begin_guest_score - final_total_slice_set[i][k].begin_host_score <= -threshold:
                ...
                # 主队建立优势
                    ...
                    t_type = 5
                # 主队落后但缩小分差
                elif final_total_slice_set[i][k].begin_guest_score - final_total_slice_set[i][k].begin_host_score > threshold:# and final_total_slice_set[i][k].end_guest_score - final_total_slice_set[i][k].end_host_score >= 0:
                    t_type = 6
